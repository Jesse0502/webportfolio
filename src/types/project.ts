...
export const projects: Project[] = [
  ...
  "problemStatement": "Users need a centralized system to store, retrieve, and leverage their personal information across conversations with an AI assistant. Traditional chatbots lack persistent memory, making each conversation isolated. Additionally, users want to upload documents and have the AI understand their content, schedule reminders, and maintain a searchable knowledge base of facts about themselves.",
  "solution": "I built a full-stack AI memory assistant that functions as a proactive 'second brain' by integrating streaming chat, long-term memory, and document analysis into a single cohesive platform. Powered by xAI Grok-3 and LangChain, the system uses a background processor to analyze conversations in real-time, extracting atomic facts and resolving pronouns to build a persistent third-person memory profile for the user. Beyond chat, it features a comprehensive RAG pipeline that ingests and chunks files—ranging from PDFs to spreadsheets—into a Supabase vector store, allowing the AI to cross-reference uploaded data with user history. To keep users organized, an intelligent scheduler automatically detects reminders in the chat and converts them into system notifications. The entire platform is secured with user-scoped JWT authentication and wrapped in a modern, responsive frontend capable of deep linking and global search.",
  "issuesFaced": [
  ...
  "description": "RUMI is a comprehensive AI-driven networking platform that revolutionizes how professionals connect. The system uses Google Gemini AI to analyze user profiles from resumes, extracting skills, industries, interests, and personality traits. It employs a hybrid matching algorithm combining semantic similarity (Sentence Transformers embeddings) with weighted boosts for shared skills, industries, and location. The platform features an interactive AI chat interface where users describe who they're looking for, and the AI asks clarifying questions before finding matches. Additional features include a letter writing system with AI-powered recipient matching, event creation with intelligent attendee suggestions, real-time messaging via WebSockets, group chats for events, and a comprehensive connection management system. The backend is built with FastAPI and MongoDB, using Google Cloud Storage for file uploads, while the frontend is a modern React/TypeScript application with Redux state management and TailwindCSS styling.",
  "problemStatement": "Traditional networking platforms match users based on superficial criteria like job titles or company names, missing the deeper connections based on actual skills, interests, and synergies. Users spend hours manually searching for people with specific expertise or services, and there's no intelligent way to discover connections beyond keyword matching. The networking process is time-consuming, lacks context awareness, and doesn't leverage AI to understand user intent and preferences.",
  "solution": "Built an AI-powered networking platform that uses natural language processing and semantic embeddings to understand user profiles and preferences at a deeper level. The system extracts meaningful attributes from resumes using Google Gemini AI, creates vector embeddings for semantic search, and uses a hybrid matching algorithm that combines semantic similarity with weighted boosts for shared attributes. This transforms networking from manual searching to intelligent, context-aware recommendations.",
